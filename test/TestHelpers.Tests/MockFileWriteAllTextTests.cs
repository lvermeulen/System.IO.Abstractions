...
namespace System.IO.Abstractions.TestHelpers.Tests
{
    ...
    public class MockFileWriteAllTextTests {
        ...
        public void MockFile_WriteAllText_ShouldNotThrowAnArgumentNullExceptionIfTheContentIsNull()
        {
            ...
            MockFileData data = fileSystem.GetFile(filePath);
            Assert.Equal(data.Contents, new byte[] {});
        }
        ...
        public void MockFile_WriteAllText_ShouldThrowAnUnauthorizedAccessExceptionIfTheFileIsReadOnly()
        {
            ...
            MockFileSystem fileSystem = new MockFileSystem();
            string filePath = XFS.Path(@"c:\something\demo.txt");
            MockFileData mockFileData = new MockFileData(new byte[0]) {Attributes = FileAttributes.ReadOnly};
        ...
        public static IEnumerable<object[]> GetEncodingsWithExpectedBytesObjects()
        {
            foreach (var item in GetEncodingsWithExpectedBytes())
            {
        ...
        [Theory]
        [MemberData(nameof(GetEncodingsWithExpectedBytesObjects))]
        ...
        public void MockFile_WriteAllTextMultipleLines_ShouldWriteTextFileToMemoryFileSystem()
        {
            ...
            List<string> fileContent = new List<string> {"Hello there!", "Second line!"};
            string expected = "Hello there!" + Environment.NewLine + "Second line!" + Environment.NewLine;
